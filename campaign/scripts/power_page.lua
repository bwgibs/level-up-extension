...
function onInit()
	...
	DB.addHandler(DB.getPath(node, "powermeta.pactmagiclevel"), "onUpdate", onUsesChanged);
...
function onClose()
	...
	DB.removeHandler(DB.getPath(node, "powermeta.pactmagiclevel"), "onUpdate", onUsesChanged);
...
function updatePowerWindowUses(nodeChar, w)
	...
	if bCaster then
		if nLevel >= 1 and nLevel <= PowerManager.SPELL_LEVELS then
			...
			nTotalCast = (aCharSlots[nLevel].nTotalCast or 0);			-- Number of spell slots checked off after casting, still 0 even if #available = 0
			nTotalPrepared = (aCharSlots[nLevel].nTotalPrepared or 0);	-- How many spells in a group are checked off as prepared
		end
	elseif rGroup then
		...
		if sMode == "combat" then
			if nLevel > 0 then
				if rGroup.nPrepared > 0 and nPrepared <= 0 then
					...
					-- Level up
					local nSpellPoints = 0;
					if aCharSlots["pactmagic"].nMax > 0 then
						nSpellPoints = aCharSlots["pactmagic"].nMax - aCharSlots["pactmagic"].nTotalCast;
					end
					for i = nLevel, PowerManager.SPELL_LEVELS do
						...
						-- Level Up
						if not bValidSlot and nSpellPoints > 0 then
							local nMaxLevel = 0;
							if i < 3 then
								nMaxLevel = math.floor(nSpellPoints / (i + 1));
							elseif i < 6 then
								nMaxLevel = math.floor(nSpellPoints / (i + 2));
							end
							if nMaxLevel > 0 and DB.getValue(nodeChar, "powermeta.pactmagiclevel", 0) >= i then
								bValidSlot = true;
								break;
							end
						end
		...
		if rGroup then
			...
			if rGroup.grouptype == "" then
				...
				if nLevel >= 1 and nLevel <= PowerManager.SPELL_LEVELS then
					...
					aCharSlots[nLevel].nTotalCast = DB.getValue(nodeChar, "powermeta.spellslots" .. nLevel .. ".used", 0);
				end
				-- Level Up
				aCharSlots["pactmagic"].nTotalCast = DB.getValue(nodeChar, "powermeta.pactmagicpoints.used", 0);
			end
		end
	end
...
function rebuildGroups()
	...
	for i = 1, PowerManager.SPELL_LEVELS do
		aCharSlots[i] = { nMax = DB.getValue(nodeChar, "powermeta.spellslots" .. i .. ".max", 0) };		-- Level Up
	end
	-- Level Up
	aCharSlots["pactmagic"] = { nMax = DB.getValue(nodeChar, "powermeta.pactmagicpoints.max", 0) };
end
